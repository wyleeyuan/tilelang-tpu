...
extern "C" int tilelang_tpu_run(void** args) {
  ...
  size_t expanded_scale_size = 6144 * 12 * 2;
  char* q_full = static_cast<char*>(args[3]);
  ...
  int rst = main_kernel((unsigned long long)dev_query, (unsigned long long)dev_wuq, (unsigned long long)dev_expanded_scale, (unsigned long long)dev_q_full);
  ...
  for (int i = 0; i < warmup_runs; i++) {
      rst = main_kernel((unsigned long long)dev_query, (unsigned long long)dev_wuq, (unsigned long long)dev_expanded_scale, (unsigned long long)dev_q_full);
  }
  ...
  // 拷贝输出数据回主机
  tpuRtMemcpyD2S(q_full, dev_q_full, q_full_size);
