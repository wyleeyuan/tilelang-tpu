...
extern "C" int tilelang_tpu_run(void** args) {
  char* q = static_cast<char*>(args[0]);
  size_t q_size = 1 * 32 * 1 * 512 * 2;
  char* q_rope = static_cast<char*>(args[1]);
  size_t q_rope_size = 1 * 32 * 1 * 64 * 2;
  char* q_cos = static_cast<char*>(args[2]);
  size_t q_cos_size = 1 * 32 * 1 * 64 * 2;
  char* q_sin = static_cast<char*>(args[3]);
  size_t q_sin_size = 1 * 32 * 1 * 64 * 2;
  char* kv_cache = static_cast<char*>(args[4]);
  size_t kv_cache_size = 32 * 512 * 2;
  char* pe_cache = static_cast<char*>(args[5]);
  size_t pe_cache_size = 32 * 64 * 2;
  char* kv_new = static_cast<char*>(args[6]);
  size_t kv_new_size = 1 * 512 * 2;
  char* pe_new = static_cast<char*>(args[7]);
  size_t pe_new_size = 1 * 64 * 2;
  char* k_cos = static_cast<char*>(args[8]);
  size_t k_cos_size = 1 * 64 * 2;
  char* k_sin = static_cast<char*>(args[9]);
  size_t k_sin_size = 1 * 64 * 2;
  char* block_table = static_cast<char*>(args[10]);
  size_t block_table_size = 2 * 1 * 4;
  char* save_slots = static_cast<char*>(args[11]);
  size_t save_slots_size = 1 * 1 * 4;
  char* kv_gather = static_cast<char*>(args[12]);
  size_t kv_gather_size = 32 * 512 * 2;
  char* pe_gather = static_cast<char*>(args[13]);
  size_t pe_gather_size = 32 * 64 * 2;
  char* pe_new_rope = static_cast<char*>(args[14]);
  size_t pe_new_rope_size = 1 * 64 * 2;
  char* output = static_cast<char*>(args[15]);
  size_t output_size = 1 * 32 * 1 * 512 * 2;
  ...
  // 分配设备内存
  tpuRtMalloc((void **)(&dev_q), q_size, 0);
  tpuRtMalloc((void **)(&dev_q_rope), q_rope_size, 0);
  tpuRtMalloc((void **)(&dev_q_cos), q_cos_size, 0);
  tpuRtMalloc((void **)(&dev_q_sin), q_sin_size, 0);
  ...
  tpuRtMalloc((void **)(&dev_k_cos), k_cos_size, 0);
  tpuRtMalloc((void **)(&dev_k_sin), k_sin_size, 0);
  ...
  // 拷贝数据到设备
  tpuRtMemcpyS2D(dev_q, q, q_size);
  tpuRtMemcpyS2D(dev_q_rope, q_rope, q_rope_size);
  tpuRtMemcpyS2D(dev_q_cos, q_cos, q_cos_size);
  tpuRtMemcpyS2D(dev_q_sin, q_sin, q_sin_size);
  ...
  tpuRtMemcpyS2D(dev_k_cos, k_cos, k_cos_size);
  tpuRtMemcpyS2D(dev_k_sin, k_sin, k_sin_size);
  ...
  int rst = main_kernel((unsigned long long)dev_q, (unsigned long long)dev_q_rope, (unsigned long long)dev_q_cos, (unsigned long long)dev_q_sin, (unsigned long long)dev_kv_cache, (unsigned long long)dev_pe_cache, (unsigned long long)dev_kv_new, (unsigned long long)dev_pe_new, (unsigned long long)dev_k_cos, (unsigned long long)dev_k_sin, (unsigned long long)dev_block_table, (unsigned long long)dev_save_slots, (unsigned long long)dev_kv_gather, (unsigned long long)dev_pe_gather, (unsigned long long)dev_pe_new_rope, (unsigned long long)dev_output);
  ...
  for (int i = 0; i < warmup_runs; i++) {
      rst = main_kernel((unsigned long long)dev_q, (unsigned long long)dev_q_rope, (unsigned long long)dev_q_cos, (unsigned long long)dev_q_sin, (unsigned long long)dev_kv_cache, (unsigned long long)dev_pe_cache, (unsigned long long)dev_kv_new, (unsigned long long)dev_pe_new, (unsigned long long)dev_k_cos, (unsigned long long)dev_k_sin, (unsigned long long)dev_block_table, (unsigned long long)dev_save_slots, (unsigned long long)dev_kv_gather, (unsigned long long)dev_pe_gather, (unsigned long long)dev_pe_new_rope, (unsigned long long)dev_output);
  }
  ...
  // 释放设备内存
  tpuRtFree(&dev_q, 0);
  tpuRtFree(&dev_q_rope, 0);
  tpuRtFree(&dev_q_cos, 0);
  tpuRtFree(&dev_q_sin, 0);
  tpuRtFree(&dev_kv_cache, 0);
  tpuRtFree(&dev_pe_cache, 0);
  tpuRtFree(&dev_kv_new, 0);
  tpuRtFree(&dev_pe_new, 0);
  tpuRtFree(&dev_k_cos, 0);
  tpuRtFree(&dev_k_sin, 0);
