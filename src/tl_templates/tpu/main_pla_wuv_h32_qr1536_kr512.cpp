...
extern "C" int tilelang_tpu_run(void** args) {
  char* latent = static_cast<char*>(args[0]);
  size_t latent_size = 1 * 32 * 1 * 512 * 2;
  ...
  // 分配设备内存
  tpuRtMalloc((void **)(&dev_latent), latent_size, 0);
  ...
  // 拷贝数据到设备
  tpuRtMemcpyS2D(dev_latent, latent, latent_size);
  ...
  int rst = main_kernel((unsigned long long)dev_latent, (unsigned long long)dev_wukv, (unsigned long long)dev_expanded_scale, (unsigned long long)dev_output);
  ...
  for (int i = 0; i < warmup_runs; i++) {
      rst = main_kernel((unsigned long long)dev_latent, (unsigned long long)dev_wukv, (unsigned long long)dev_expanded_scale, (unsigned long long)dev_output);
  }
